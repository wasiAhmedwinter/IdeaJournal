...
ACTIVE_FILE = os.path.join(BASE, 'analytics', 'active.json')
...
def track_active_users():
    ...
    try:
        ...
        active[session['user']] = datetime.now().isoformat()
...
def login():
    ...
    if username == ADMIN_USERNAME and password == ADMIN_PASSWORD:
        ...
        log_login_event(username, 'admin')
    ...
    log_login_event(username, session['role'])
    session['last_seen'] = datetime.now().isoformat()
    return redirect('/dashboard')
...
def admin_stats():
    users = load_users()
    total_users = len([u for u in users if u != "_meta"])

    return jsonify({
        "total_users": total_users,
        "total_ideas": count_total_ideas(),
        "active_users": get_active_user_count(),
        "logins": get_login_logs()
    })
